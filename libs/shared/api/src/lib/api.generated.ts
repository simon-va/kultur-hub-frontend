...
export class EventClient {
     ...
     * @return OK
     */
    getEventsOverview(organisationId: string): Observable<EventOverviewResponse[]> {
        let url_ = this.baseUrl + "/organisations/{organisationId}/events/overview";
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processGetEventsOverview(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGetEventsOverview(response_ as any);
                } catch (e) {
                    return _observableThrow(e) as any as Observable<EventOverviewResponse[]>;
                }
            } else
                return _observableThrow(response_) as any as Observable<EventOverviewResponse[]>;
        }));
    }

    protected processGetEventsOverview(response: HttpResponseBase): Observable<EventOverviewResponse[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
        ...
        if (status === 200) {
            ...
            if (Array.isArray(resultData200)) {
                result200 = [] as any;
                for (let item of resultData200)
                    result200!.push(EventOverviewResponse.fromJS(item));
            }
            else {
                result200 = null as any;
            }
...
export class EventOverviewResponse implements IEventOverviewResponse {
    id!: string;
    ...
    constructor(data?: IEventOverviewResponse) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (this as any)[property] = (data as any)[property];
    ...
    static fromJS(data: any): EventOverviewResponse {
        data = typeof data === 'object' ? data : {};
        let result = new EventOverviewResponse();
        result.init(data);
        return result;
    }
